...
Letter *BubbleSortFreq(Letter *letters)
{
...
void BubbleSortLet(Letter *letters)
{
	for (Letter *i = letters; i != NULL; i = i->next)
	{
		for (Letter *j = i; j != NULL; j = j->next)
...
TREE *BuildShenon(Letter *a)
{
	if (a != NULL)
	{
		...
		else
		{
			...
			head->left = BuildShenon(first);
			head->right = BuildShenon(second);
		}
		return head;
	}
	else return NULL;
...
void TraverseTREE(TREE *T, int deph, CODE *Codes, char *str, int *num, int codes_size)
{
	if (T != NULL)
	{
		if (!T->left || !T->right)
		{
			...
			else printf("Ошибка в создании кодов.\n");
